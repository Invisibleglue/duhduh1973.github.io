...
var Amplitude = (function () {
    ...
    function publicGetUpdateTime(){
        return config.active_song.currentTime;
    }

    function publicGetActiveVisualization(){
        return config.active_metadata.visualization;
    }
    ...
    function privateSkipSongToLyric(){
        if( !config.active_metadata.live ){
            config.active_song.currentTime = (29);
        }
    }
    ...
    function privateSkipClickHandle(){
        privateSkipSongToLyric();
    }
    ...
    function privateCheckSongVisualization(){
        ...
        // console.log(config.active_metadata.visualization);
        return changed;
    }
    ...
    function privateStartVisualization(){
        ...
        privateRunCallback('visual_started');
    }
    ...
    function privateInitializeEventHandlers(){
        ...
        var skip_classes = document.getElementsByClassName("amplitude-skip");

        for( var i = 0; i < skip_classes.length; i++ ){
            if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
                skip_classes[i].addEventListener('touchstart', privateSkipClickHandle );
            }else{
                skip_classes[i].addEventListener('click', privateSkipClickHandle );
            }
        }
    ...
    /*function timeEvent() {
        if (config.active_song.currentTime >= 33 && config.active_song.currentTime <= 34) {
            ...
            document.getElementById("#wayoutlyric1").classList.add("invisible");
        } if (config.active_song.currentTime >= 37 && config.active_song.currentTime <= 38){
            document.getElementById("#wayoutlyric2").classList.remove("invisible");
        } if (config.active_song.currentTime >= 39 && config.active_song.currentTime <= 40) {
            document.getElementById("#wayoutlyric2").classList.add("invisible");
        } if (config.active_song.currentTime >= 42 && config.active_song.currentTime <= 43) {
            document.getElementById("#wayoutlyric3").classList.remove("invisible");
        } if (config.active_song.currentTime >= 45 && config.active_song.currentTime <= 46) {
            document.getElementById("#wayoutlyric3").classList.add("invisible");
        } if (config.active_song.currentTime >= 47 && config.active_song.currentTime <= 48) {
            document.getElementById("#wayoutlyric4").classList.remove("invisible");
        } if (config.active_song.currentTime >= 50 && config.active_song.currentTime <= 51) {
    ...
    return {
        ...
        getUpdateTime: publicGetUpdateTime,
        getActiveVisualization: publicGetActiveVisualization,
